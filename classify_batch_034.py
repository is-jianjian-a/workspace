...
def classify_post(post):
    ...
    # 针对AI视频创作类内容的特殊处理
    # 标题含教程/攻略/教学/保姆级/手把手/工作流等强教学关键词 → tutorial
    tutorial_title_keywords = ["教程", "攻略", "教学", "保姆级", "手把手", "工作流", "制作流程", "全流程"]
    if any(kw in title for kw in tutorial_title_keywords):
        return "tutorial", 0.9, "标题明确为教程/攻略/教学类内容"

    # 标题含"揭秘"/"分享" + AI工具/技术 → real_ugc（经验分享）
    if any(kw in title for kw in ["分享", "揭秘", "总结", "盘点"]):
        if any(kw in title for kw in ["AI", "ai"]):
            return "real_ugc", 0.85, "AI相关经验分享/揭秘/总结类内容"

    # 标题含"学习"/"来学" + 工具/技术 → tutorial
    if any(kw in title for kw in ["学习", "来学", "教你", "解决"]):
        if any(kw in title for kw in ["AI", "ai", "Blender", "视频"]):
            return "tutorial", 0.85, "AI/视频技术教学内容"
    ...
    # 8. 真实UGC检测 (real_ugc)
    if is_real_ugc(full_text):
        return "real_ugc", 0.85, "消费者真实声音，第一人称视角，具体使用细节，真实情感"

    # 默认 fallback
    return smart_fallback(full_text)
...
def smart_fallback(text):
    ...
    if has_first_person and (has_feeling or ugc_detail_count >= 2 or has_emotion):
...
def is_invalid(content, title):
    ...
    # 只有标题、内容为空或极少（如只有"-"）
    if not content_stripped or content_stripped == "-" or len(content_stripped) < 3:
        # 如果标题本身有足够信息量，不算invalid，走info_missing
...
def is_news(text):
    ...
    return count >= 2 and not has_guide
...
def main():
    input_path = "/Users/zhijian/workspace/ai-demand-research-v3/projects/看视频时想要的AI功能-2026-05-18/02-cleaned/batches/batch_034.json"
    output_path = "/Users/zhijian/workspace/result_batch_034.json"
    ...
    for cat, count in sorted(category_counts.items(), key=lambda x: -x[1]):
        ...
        print(f"   {cat:20s} ({name:8s}): {count} 条")

    print("\n📋 分类详情:")
    for r in results:
        title_preview = r['title'][:50] + "..." if len(r['title']) > 50 else r['title']
        print(f"  [{r['category']:12s}] {title_preview}")
