...
def classify_item(item):
    ...
    personal_experience_markers = [
        ...
        "个人感受", "心得", "体会", "发现", "注意到", "观察到", "真的", "确实", "明显", "偏向"
    ]
    ...
    has_specific_data = any(char.isdigit() for char in content) and any(
        kw in text for kw in ["万", "像素", "mah", "w", "hz", "gb", "英寸", "mm", "g", "元", "nit", "倍", "mp", "分", "小时", "分钟", "天", "年"]
    )
    ...
    comparison_markers = ["对比", "vs", "🆚", "区别", "差异", "升级", "横向对比"]
    ...
    # 参数对比兜底3：标题明确为推荐/横向对比+大量参数罗列+结构化格式（如2025年手机推荐）
    if ("推荐" in title or "横向对比" in title or "对比" in title) and is_spec_heavy:
        if bullet_points >= 5 or structured_format:
            return "spec_comparison", CATEGORY_MAP["spec_comparison"], "以参数规格为核心的横向对比/推荐内容"
    ...
    # 5b. news - 市场份额/行业数据类（无个人体验，纯数据展示）
    if not has_personal and any(kw in text for kw in ["市场份额", "销量", "均价", "数据来源", "高市场份额"]):
        if any(kw in text for kw in ["图一", "图二", "数据", "来源"]):
            return "news", CATEGORY_MAP["news"], "客观展示市场/行业数据，无个人体验"

    # 5c. news - 超短内容+行业数据引用+无个人体验
    if not has_personal and len(content.strip()) < 80:
        if any(kw in text for kw in ["市场份额", "销量", "均价", "数据来源", "高市场"]):
            return "news", CATEGORY_MAP["news"], "客观展示市场/行业数据，无个人体验"

    # 5d. real_ugc - 有个人体验词（"偏向", "我觉得"等）的对比体验 → 真实UGC
    if has_personal and is_comparison:
        return "real_ugc", CATEGORY_MAP["real_ugc"], "有个人主观体验和观点的真实对比"
    ...
    # 7b. info_missing - 标题是问句但内容极少，只有重复标题或标签
    if len(content.strip()) < 30 and ("?" in title or "？" in title or "吗" in title or "纠结" in title):
        if content.strip() == title.strip() or len(content.strip()) < 20:
            return "info_missing", CATEGORY_MAP["info_missing"], "标题为疑问/纠结，但内容极少或仅重复标题"

    # 7c. info_missing - 内容几乎只有话题标签
    if len(content.strip()) < 50 and content.count("#") >= 3 and len(content.replace("#", "").strip()) < 20:
        return "info_missing", CATEGORY_MAP["info_missing"], "内容几乎只有话题标签，无实质信息"
...
def main():
    input_path = "/Users/zhijian/workspace/mind-mining/v1.0/batches/batch_035.json"
    output_path = "/Users/zhijian/workspace/result_batch_035.json"
