...
try:
    ...
    pass  # xhshow not available, skip patch
...
async def sign_with_browser(
    ...
    """
    使用浏览器内执行生成小红书签名
    通过调用网页中已有的 window._webmsxyw 函数计算签名
    """
    content_string = _build_sign_string(uri, data, method)
    
    # 调用小红书网页中已有的签名函数 window._webmsxyw
    sign_js = """
    (args) => {
        const [contentString] = args;
        try {
            if (window._webmsxyw) {
                const result = window._webmsxyw(contentString);
                return result;
            }
            return null;
        } catch (e) {
    ...
    try:
        result = await page.evaluate(sign_js, [content_string])
        if result and not result.get("error"):
            # result 格式: {"X-s": "...", "X-t": "..."}
            return {
                "x-s": result.get("X-s", result.get("x-s", "")),
                "x-t": str(result.get("X-t", result.get("x-t", int(time.time())))),
                "x-s-common": "",  # 浏览器签名不生成 x-s-common
    ...
    # 如果浏览器签名失败，返回空值
    return {"x-s": "", "x-t": str(int(time.time())), "x-s-common": "", "x-b3-traceid": get_trace_id()}
...
def sign_with_xhshow(
    ...
    try:
        ...
        # Fallback: return empty sign, will use browser-based signing
        # Note: This requires async context to use sign_with_browser
        return {"x-s": "", "x-t": str(int(time.time())), "x-s-common": "", "x-b3-traceid": get_trace_id()}
