...
class XiaoHongShuClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    async def _pre_headers(self, url: str, params: Optional[Dict] = None, payload: Optional[Dict] = None) -> Dict:
        ...
        # 备用方案：使用浏览器内执行签名（兼容Python 3.9）
        try:
            from .playwright_sign import sign_with_browser
            signs = await sign_with_browser(
            ...
            utils.logger.info(f"[XiaoHongShuClient._pre_headers] Browser sign result: x-s={'present' if signs.get('x-s') else 'missing'}")
            if signs.get("x-s"):
                headers = {
                    "X-S": signs["x-s"],
                    "X-T": signs["x-t"],
            ...
            utils.logger.error(f"[XiaoHongShuClient._pre_headers] Browser sign failed: {e}")
            pass

        # 如果都失败了，返回原始headers（会触发风控，但至少能继续）
        return self.headers
    ...
    async def pong(self) -> bool:
        ...
        # 强制跳过登录状态检查，直接返回True
        # 用户确认浏览器中已登录，不需要API验证
        utils.logger.info("[XiaoHongShuClient.pong] Skipping login check, assuming logged in")
        return True
