...
from .browser_search import search_and_extract_via_browser
...
class XiaoHongShuCrawler(AbstractCrawler):
            ...
            while (page - start_page + 1) * xhs_limit_count <= config.CRAWLER_MAX_NOTES_COUNT:
                ...
                try:
                    ...
                    try:
                        ...
                        if notes_res and notes_res.get("items"):
                            api_success = True
                            utils.logger.info(f"[XiaoHongShuCrawler.search] API search success, got {len(notes_res.get('items', []))} items")
                    except Exception as e:
                        utils.logger.warning(f"[XiaoHongShuCrawler.search] API search failed: {e}, falling back to browser...")
                    
                    # API失败时，使用纯浏览器模拟
                    if not api_success:
                        utils.logger.info("[XiaoHongShuCrawler.search] Using browser simulation...")
                        browser_notes = await search_and_extract_via_browser(
                            page=self.context_page,
                            keyword=keyword,
                            max_notes=config.CRAWLER_MAX_NOTES_COUNT,
                            max_comments_per_note=config.CRAWLER_MAX_COMMENTS_COUNT_SINGLENOTES,
                        )
                        # 转换为统一格式并保存
                        for note in browser_notes:
                            ...
                            if note.get('comments'):
                                for comment in note['comments']:
                                    await xhs_store.update_xhs_note_comment(note['note_id'], comment)
                        
                        utils.logger.info(f"[XiaoHongShuCrawler.search] Browser simulation got {len(browser_notes)} notes with comments")
    ...
    async def batch_get_note_comments(self, note_list: List[str], xsec_tokens: List[str]):
        ...
        for index, note_id in enumerate(note_list):
            try:
                await self.get_comments(note_id=note_id, xsec_token=xsec_tokens[index])
            except Exception as e:
                utils.logger.warning(f"[XiaoHongShuCrawler.batch_get_note_comments] API comments failed for {note_id}: {e}")
                # 浏览器回退获取评论
                try:
                    browser_comments = await get_note_comments_via_browser(
                        page=self.context_page,
                        note_id=note_id,
                    ...
                    if browser_comments:
                        utils.logger.info(f"[XiaoHongShuCrawler.batch_get_note_comments] Browser got {len(browser_comments)} comments for {note_id}")
                        # 保存浏览器获取的评论
                        for comment in browser_comments:
                            await xhs_store.update_xhs_note_comment(note_id, comment)
                except Exception as be:
                    utils.logger.error(f"[XiaoHongShuCrawler.batch_get_note_comments] Browser comments also failed: {be}")
    ...
    async def create_xhs_client(self, httpx_proxy: Optional[str]) -> XiaoHongShuClient:
        ...
        # Fix: CDP模式下，使用urls过滤可能导致cookie获取不完整
        # 先尝试获取所有cookie，如果为空再使用url过滤
        cookie_str, cookie_dict = await utils.convert_browser_context_cookies(
            self.browser_context,
            urls=None,  # 获取所有cookie，不限制url
        )
        utils.logger.info(f"[XiaoHongShuCrawler.create_xhs_client] Got {len(cookie_dict)} cookies from browser context")
        if not cookie_dict.get("web_session"):
            utils.logger.warning("[XiaoHongShuCrawler.create_xhs_client] web_session not found in all cookies, trying with url filter...")
            cookie_str, cookie_dict = await utils.convert_browser_context_cookies(
                self.browser_context,
                urls=self.cookie_urls,
            )
